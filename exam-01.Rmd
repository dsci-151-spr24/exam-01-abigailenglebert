...
***The airlines that have the most flights departing from NYC airports in 2013 are United Airlines(UA), JetBlue(B6), and ExpressJet Airlines(EV). For this code chunk I used the count function, so it would properly organize the chart, then I imputed the name for number of flights per airline. This way it would be a simple chart to just show me the number of flights that departed from NYC in 2013.***
...
***The carrier with the highest mean arrival delay was Frontier Airlines with a delay of 21.9. In this chunk I used a code that would look through the entire dataset for the airline with the highest mean arrival delay. This just made it easier so I didnt have to search through the dataset for the airline.***
